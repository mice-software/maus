...
minor_verison=5
patch_version=2
version=${major_version}.${minor_verison}.${patch_version}
directory=cmake-${version}
filename=${directory}.tar.gz
url=https://cmake.org/files/v${major_version}.${minor_verison}/${filename}

echo
echo 'INFO: Installing third party library CMake'
echo '-------------------------------------------'
echo
...
if [ -n "${MAUS_ROOT_DIR+x}" ]; then
    ...
    then
      ...
      echo
      sleep 1
      ./bootstrap --prefix=${MAUS_ROOT_DIR}/third_party/install/

      echo
      echo "INFO: Making:"
      echo
      sleep 1
      make -j$MAUS_NUM_THREADS || { echo "FAIL: Failed to configure/make"; exit 1; }
