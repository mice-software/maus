#!/usr/bin/env bash
version=2.4.1
directory=scons-${version}
filename=${directory}.tar.gz
url="http://downloads.sourceforge.net/project/scons/scons/${version}/${filename}"

echo
echo 'INFO: Installing third party library SCons 2.4.1'
echo '------------------------------------------------'
echo
...
if [ -n "${MAUS_ROOT_DIR+x}" ]; then
    ...
    then
      ...
      echo
      sleep 1
      python setup.py install --prefix=${MAUS_ROOT_DIR}/third_party/install/
    # Create a link to the SCons directory not based on version number
      cd ${MAUS_ROOT_DIR}/third_party/build/
      ln -fs ${directory} scons
