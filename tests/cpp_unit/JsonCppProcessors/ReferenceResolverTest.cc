...
TEST(ReferenceResolverJsonToCppTest, ResolverTest) {
    ...
    RefManager::Death();
    RefManager::Birth();
    EXPECT_THROW(res.ResolveReferences(), Exception);
...
TEST(ReferenceResolverJsonToCppTest, VectorResolverTest) {
    ...
    EXPECT_THROW(res2.ResolveReferences(), Exception);
    EXPECT_THROW(res.ResolveReferences(), Exception);
...
TEST(ReferenceResolverJsonToCppTest, RefManagerBirthTest) {
    ...
    EXPECT_THROW(RefManager::Birth(), Exception);
    RefManager::Death();
    EXPECT_FALSE(RefManager::HasInstance());
    EXPECT_THROW(RefManager::Death(), Exception);
    EXPECT_THROW(RefManager::GetInstance(), Exception);
    RefManager::Birth();
...
TEST(ReferenceResolverCppToJsonTest, TypedResolverTest) {
    ...
    EXPECT_THROW(RefManager::GetInstance().
                           SetPointerAsValue(&child, "PATH TO BRANCH"), Exception);
    ...
    EXPECT_THROW(res.ResolveReferences(parent), Exception);
    ...
    RefManager::GetInstance().SetPointerAsValue(null, "SHOULD IGNORE");
    EXPECT_THROW(RefManager::GetInstance().GetPointerAsValue(null), Exception);
