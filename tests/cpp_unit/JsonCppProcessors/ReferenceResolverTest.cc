...
TEST(ReferenceResolverJsonToCppTest, ResolverTest) {
    ...
    RefManager::Death();
    RefManager::Birth();
    EXPECT_THROW(res.ResolveReferences(), Exceptions::Exception);
...
TEST(ReferenceResolverJsonToCppTest, VectorResolverTest) {
    ...
    EXPECT_THROW(res2.ResolveReferences(), Exceptions::Exception);
    EXPECT_THROW(res.ResolveReferences(), Exceptions::Exception);
...
TEST(ReferenceResolverJsonToCppTest, RefManagerBirthTest) {
    ...
    EXPECT_THROW(RefManager::Birth(), Exceptions::Exception);
    RefManager::Death();
    EXPECT_FALSE(RefManager::HasInstance());
    EXPECT_THROW(RefManager::Death(), Exceptions::Exception);
    EXPECT_THROW(RefManager::GetInstance(), Exceptions::Exception);
    RefManager::Birth();
...
TEST(ReferenceResolverCppToJsonTest, TypedResolverTest) {
    ...
    EXPECT_THROW(RefManager::GetInstance().
                           SetPointerAsValue(&child, "PATH TO BRANCH"), Exceptions::Exception);
    ...
    EXPECT_THROW(res.ResolveReferences(parent), Exceptions::Exception);
    ...
    RefManager::GetInstance().SetPointerAsValue(null, "SHOULD IGNORE");
    EXPECT_THROW(RefManager::GetInstance().GetPointerAsValue(null), Exceptions::Exception);
