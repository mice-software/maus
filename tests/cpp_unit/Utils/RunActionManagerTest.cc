...
TEST(RunActionManagerTest, TestPushBack) {
    ...
    EXPECT_THROW(data_run->PushBack(NULL), Exception);
    delete data_run;

    RunActionTest* data_run_action = new RunActionTest();
    data_run = new RunActionManager();
    data_run->PushBack(data_run_action);
    EXPECT_THROW(data_run->PushBack(data_run_action), Exception);
    delete data_run;
}
...
TEST(RunActionManagerTest, TestSwap) {
    ...
    EXPECT_THROW(data_run->Swap(run_action_1, run_action_3), Exception);
    data_run->PushBack(run_action_3);
    // run_action_3 is already in data_run
    EXPECT_THROW(data_run->Swap(run_action_2, run_action_3), Exception);
    delete data_run;
    delete run_action_1;
}
