...
TEST(JsonWrapperTest, StringToJson) {
  ...
  std::string bad_json = "{";
  EXPECT_THROW(JsonWrapper::StringToJson(bad_json), MAUS::Exceptions::Exception);
}
...
TEST(JsonWrapperTest, GetItemTest) {
  ...
  for (unsigned int i = 0; i < 2; ++i) {
    ...
    EXPECT_THROW( // bad type
     JsonWrapper::GetItem(good_val, i, JsonWrapper::stringValue), MAUS::Exceptions::Exception);
  }
  Json::Value empty_array =  JsonWrapper::StringToJson("[]");
  EXPECT_THROW( // empty value
    JsonWrapper::GetItem(empty_array, 0, JsonWrapper::anyValue), MAUS::Exceptions::Exception);
  EXPECT_THROW( // out of range item
    JsonWrapper::GetItem(good_val, 4, JsonWrapper::anyValue), MAUS::Exceptions::Exception);
    ...
    for (unsigned int j = 0; j < 5; ++j) {
      if ( i == j || (i == 1 && j == 0)) {
          ...
          EXPECT_THROW(JsonWrapper::GetProperty(good_val, gets[i], types[j]),
                       MAUS::Exceptions::Exception)  << i << " " << j << " " << gets[i];
      }
    }
...
TEST(JsonWrapperTest, TypeConversionTest) {
  ...
  EXPECT_THROW(JsonWrapper::JsonTypeToValueType(JsonWrapper::anyValue), MAUS::Exceptions::Exception);
}
...
TEST(JsonWrapperTest, ObjectMergeTest) {
  ...
  EXPECT_THROW(JsonWrapper::ObjectMerge(object_1, object_1), MAUS::Exceptions::Exception);
  // throw because non-objects
  EXPECT_THROW(JsonWrapper::ObjectMerge(int_1, object_1), MAUS::Exceptions::Exception);
  EXPECT_THROW(JsonWrapper::ObjectMerge(object_1, int_1), MAUS::Exceptions::Exception);
}
...
TEST(JsonWrapperTest, ArrayMergeTest) {
  ...
  // non-array in merge
  EXPECT_THROW(JsonWrapper::ArrayMerge(int_1, array_1), MAUS::Exceptions::Exception);
  EXPECT_THROW(JsonWrapper::ArrayMerge(array_1, int_1), MAUS::Exceptions::Exception);
}
...
TEST(JsonWrapperTest, PathHandlingTest) {
  ...
  EXPECT_THROW(JsonWrapper::Path::AppendPath(test, "append/"), MAUS::Exceptions::Exception);
  Json::Value test_2;
...
TEST(JsonWrapperTest, PathDereferenceTest) {
  ...
  EXPECT_THROW(JsonWrapper::Path::DereferencePath(not_branch, "#1"), MAUS::Exceptions::Exception);
  // branch does not exist
  EXPECT_THROW(JsonWrapper::Path::DereferencePath(test, "#no_branch"), MAUS::Exceptions::Exception);
  // array element does not exist
  EXPECT_THROW(JsonWrapper::Path::DereferencePath(test, "#object_0/2"), MAUS::Exceptions::Exception);
}
