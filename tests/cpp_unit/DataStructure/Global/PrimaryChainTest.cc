...
TEST_F(PrimaryChainTestDS, test_Throws) {
  ...
  ASSERT_THROW(_primary_chain0->AddTrack(t6, t7), Exception);
  ASSERT_THROW(_primary_chain0->AddPrimaryTrack(t6), Exception);

  ASSERT_NO_THROW(_primary_chain0->AddTrack(t7, t6));

  ASSERT_THROW(_primary_chain0->AddTrack(t9, t8), Exception);
}
} // ~namespace MAUS
