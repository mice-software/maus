...
TEST_F(BTFieldConstructorTest, EndFieldTest) {
  ...
  EXPECT_THROW(delete _field->GetEndFieldModel(_mod[0], 1), Exception);
  _mod[0]->setProperty<std::string>("EndFieldType","Tanh");

  // tanh //
  _mod[1]->setProperty<double>("EndLength", 0);
  EXPECT_THROW(delete _field->GetEndFieldModel(_mod[1], 1), Exception);
  ...
  EXPECT_THROW(delete _field->GetEndFieldModel(_mod[1], 2), Exception);
  _mod[1]->setProperty<int>("MaxEndPole", 3);

  // enge //
  _mod[2]->setProperty<double>("EndLength", 0);
  EXPECT_THROW(delete _field->GetEndFieldModel(_mod[2], 1), Exception);
  ...
  EXPECT_THROW(delete _field->GetEndFieldModel(_mod[2], 2), Exception);
...
TEST_F(BTFieldConstructorTest, GetMultipoleTest) {
  ...
  EXPECT_THROW(delete _field->GetMultipole(_mod[0]), Exception);
...
TEST_F(BTFieldConstructorTest, GetCombinedFunctionTest) {
  ...
  //This is a UI so check for invalid input
  _mod[0]->setProperty<int>("Pole", 0);
  EXPECT_THROW(delete _field->GetCombinedFunction(_mod[0]), Exception);
  _mod[0]->setProperty<int>("Pole", 1);
...
TEST_F(BTFieldConstructorTest, GetSectorMagneticFieldMapTest) {
  ...
  EXPECT_THROW(_field->GetField(_mod[0]), Exception);
