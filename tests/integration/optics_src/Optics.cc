...
void Envelope(const MiceModule* root, std::vector<CovarianceMatrix>& env_out, std::vector<TransferMap*>& tm_out)
{
  ...
  for(unsigned int i=0; i<env_out.size(); i++) Squeak::mout(Squeak::debug) << env_out[i] << std::endl;
...
template <class T> void AddHitsToMap(std::vector<T*> T_vector, std::map< StationId,   std::vector<PhaseSpaceVector> >& map)
{
  for(size_t i=0; i<T_vector.size(); i++)
  {
    StationId        id ( *(T_vector[i]) );
    PhaseSpaceVector psv( (MCHit*)T_vector[i]);
    if(map.find(id) == map.end()) 
      map[id] = std::vector<PhaseSpaceVector>();
...
std::vector<TransferMap*>      TrackingDerivativeTransferMaps(std::vector<PhaseSpaceVector> hitsIn, bool referenceOnly)
{
  ...
  for(unsigned int j=0; j<hitsIn.size() && (!referenceOnly || j==0); j++)
  {
    ...
    AddHitsToMap(event->specialHits, g_hits);
    AddHitsToMap(event->virtualHits, g_hits);
