...
def get_environment():
    ...
    # libraries
    env = SCons.Environment.Environment(SHLIBPREFIX="", BUILDERS = {'Dylib2SO' : dylib2so}) # pylint: disable-msg=E0602
    if os.path.isfile('.use_llvm_with_maus'):
        env['CC'] = "llvm-gcc"
        env['CXX'] = "llvm-g++"

    env.Tool('swig', '%s/third_party/swig-2.0.1'% os.environ['MAUS_THIRD_PARTY'])
    ...
    env.Append(LIBPATH =  libs.split(':') + ["%s/build" % os.environ['MAUS_THIRD_PARTY']])

    env.Append(CPPPATH=["%s/third_party/install/include" % os.environ['MAUS_THIRD_PARTY'], \
                          "%s/third_party/install/include/python2.7" % os.environ['MAUS_THIRD_PARTY'], \
                          "%s/third_party/install/include/root" % os.environ['MAUS_THIRD_PARTY'], \
                          "%s/src/legacy" % os.environ['MAUS_ROOT_DIR'],
			    "%s/src/common_cpp" % os.environ['MAUS_ROOT_DIR'], ""])
