...
MAUS contains several applications to perform various tasks. Two main applications are provided. \verb|bin/simulate_mice.py| makes a Monte Carlo simulation of the experiment and \verb|bin/analyze_data_offline.py| reconstructs an existing data file. To run the applications, first open a terminal window and move to the root directory of MAUS. Then type
\begin{verbatim}
 > source env.sh
...
By default, MAUS writes data as a ROOT file. ROOT \hyperref{http://root.cern.ch} is a standard package for data analysis in high energy physics. Two techniques are foreseen for accessing the data.
