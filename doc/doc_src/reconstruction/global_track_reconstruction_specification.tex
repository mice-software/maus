...
The interfaces OpticsModel, TransferMap, and TrajectoryFitter provide the
required implementation flexibility which is detailed in figure 1.2. The
particular optics model and trajectory fitting algorithm can be specified in the configuration file. The appropriate class instances will then be instantiated.
...
\begin{enumerate}
  ...
  \item a list of particle track data structures.
\end{enumerate}
...
The items of the particle track data structure that are used for inputs into
reconstruction are
\begin{enumerate}
  \item the 6D phase space coordinates of the track $(t, E, x, Px, y, Py)$, and
  \item the ID of the detector which measured the track.
\end{enumerate}
...
provided as an output of reconstruction. For measured particle tracks the
...
The outputs of reconstruction are a list of particle trajectory data structures.
Each trajectory contains
\begin{enumerate}
  \item the PDG particle ID for the type of particle associated with the trajectory, and
  \item a list of particle track data structures that comprise the discrete trajectory.
\end{enumerate}

Each particle track data structure contains the following items:
\begin{enumerate}
  \item the 6D phase space coordinates of the track $(t, E, x, Px, y, Py)$,
  \item the calculated z and Pz coordinates,
  \item the uncertainties on the phase space coordinates in the form of a
        covariance matrix.
  \item the ID of the detector which measured the track, or zero if this track was 
        calculated.
\end{enumerate}
...
Define input and output data structures. It is crucial to get this flushed out early on so that the detector groups know what is needed. Referring to the class diagram, this would involve implementing PhaseSpaceVector (exists already), ParticleTrack, Detector,
and ReconstructionInput for the inputs. ParticleTrajectory must be implemented
for the outputs.
