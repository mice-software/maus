...
EmCalChannelMap::EmCalChannelMap()
{
...
EmCalChannelMap::EmCalChannelMap(std::string fileName)
{
   fadc_boards.resize(0);
   fadc_crates.resize(0);
   fadc_channels.resize(0);
...
void EmCalChannelMap::Initialize(std::string fileName)
{
...
bool EmCalChannelMap::isThisEmCalFadc(VmefAdcHit* theHit)
{
...
int EmCalChannelMap::findThePosition(VmefAdcHit* theHit, int& cel, int& p)
{
   int crate = theHit->crate();
   int board = theHit->board();
   int chan = theHit->channel();
...
int EmCalChannelMap::findThePosition(VmefAdcHit* theHit)
{
   int crate = theHit->crate();
   int board = theHit->board();
   int chan = theHit->channel();
...
void EmCalChannelMap::Print()
{
   std::cout<<"===== EmCalChannelMap ====="<<std::endl;
   for( int j = 0; j < numchans; j++)
   {
      std::cout << "Pmt " << pmtName[j] << " " << pmtNum[j] << " fADC" 
	   << fadc_crates[j] << fadc_boards[j] << fadc_channels[j];
      std::cout << " EmCal " << cell[j] << pmt[j] << std::endl;
   }
}
