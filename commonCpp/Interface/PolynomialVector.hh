...
//! i.e. maps a vector x onto a vector y with \f$\vec{y} = a_0 + sum( a_{j_1j_2...j_n} x_1^{j1} x_2^{j2} ... x_n^{jn}  )\f$.
...
public:
    ...
    //! Make a shell of points where magnitude of each vector is delta*delta and 
    //! number of points is at least multiplier*NumberOfPolynomialCoefficients(i_order, delta.size())
    //! so that if multiplier >= 1 it will always define a polynomial of order i_order
    //! Require evenly spaced ito angle, sometimes this means I make more points on the shell
    static std::vector< std::vector<double> > PointShell(std::vector<double> delta, int i_order);
