...
#CHANGES: (Rogers) change "optional" default to True
#         (Rogers) add optionalItem list to schema
...
class JSONSchemaValidator:
  ...
  def validate_optionalItems(self, x, fieldname, schema, items=None):
    '''
    Validates that all items in the list for the given field are permitted by
    the given schema (optional means allow any number of these items but don't
    worry if the items don't exist)
    '''
    if items is not None and x.get(fieldname) is not None:
      if type(items) != types.ListType:
        items = [items] # force to list type
      value = x.get(fieldname)
      if type(value) != types.ListType:
        value = [value] # force to list type
      for val in value:
        validates_any = False # set to true if any of the optionalItems validates
        for it in items:
          try:
            self.validate(val, it)
            validates_any = True;
          except:
            pass
        if not validates_any:
          raise ValueError("Failed to validate field '%s' against optionalItems, %s" % (fieldname, str(items)))
    return x
  ...
  def __validate(self, fieldname, data, schema):  
    if schema is not None:
      if not type(schema) == types.DictType:
        raise ValueError("Schema structure is of invalid type "+str(type(schema))+"\n"+str(schema));
