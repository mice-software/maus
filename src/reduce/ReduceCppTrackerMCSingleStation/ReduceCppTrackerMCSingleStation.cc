...
#include "src/reduce/ReduceCppTrackerMCSingleStation/ReduceCppTrackerMCSingleStation.hh"

bool ReduceCppTrackerMCSingleStation::birth(std::string argJsonConfigDocument) {
  // Check if the JSON document can be parsed, else return error only
  _classname = "ReduceCppTrackerMCSingleStation";
...
*/
  ...
  _clusters.SetNameTitle("Clusters", "Clusters");
  _clusters.Branch("npe", &_npe, "npe/D");
  _clusters.Branch("plane", &_plane, "plane/I");
  _clusters.Branch("station", &_station, "station/I");
  _clusters.Branch("tracker", &_tracker, "tracker/I");
  _clusters.Branch("x", &_x, "x/D");
  _clusters.Branch("y", &_y, "y/D");
  _clusters.Branch("z", &_z, "z/D");
  ...
  _other.Branch("Tcluster_eff", &Tcluster_eff, "Tcluster_eff/D");
  _other.Branch("space_eff1", &space_eff1, "space_eff1/D");
  _other.Branch("space_eff2", &space_eff2, "space_eff2/D");
  _other.Branch("Tspace_eff", &Tspace_eff, "Tspace_eff/D");
...
std::string  ReduceCppTrackerMCSingleStation::process(std::string document) {
...
// Start MC Events==============================================================
    for (int a = 0; a < root["mc_events"].size(); a++) {
...
// Start Recon Events===========================================================
    double recon_size = root["recon_events"].size();
    double cut_recon_size = recon_size;
    int cluster_count1 = 0;
    int cluster_count2 = 0;
    double space_count1 = 0.0;
    int space_count2 = 0;
    ofstream pyfile;
  ...
  // Start Triggering Event=====================================================
      if (root["recon_events"][h]["sci_fi_event"]["sci_fi_digits"].size()) {
  ...
  // Start of Clusters==========================================================
        ...
        //   isMember("sci_fi_clusters")) {
          ...
          for (int i = 0; i < cluster0_size; i++) {
            ...
            _station = scuts["tracker0"][i]["station"].asInt();
            _plane = scuts["tracker0"][i]["plane"].asInt();
            _x = scuts["tracker0"][i]["position"]["x"].asDouble();
            _y = scuts["tracker0"][i]["position"]["y"].asDouble();
            _z = scuts["tracker0"][i]["position"]["z"].asDouble();
            _clusters.Fill();
            }

          for (int j = 0; j < cluster1_size; j++) {
            _npe = scuts["tracker1"][j]["npe"].asDouble();
            _tracker = scuts["tracker1"][j]["tracker"].asInt();
            _station = scuts["tracker1"][j]["station"].asInt();
            _plane = scuts["tracker1"][j]["plane"].asInt();
            _x = scuts["tracker1"][j]["position"]["x"].asDouble();
            _y = scuts["tracker1"][j]["position"]["y"].asDouble();
            _z = scuts["tracker1"][j]["position"]["z"].asDouble();
            _clusters.Fill();
            }
        // }
  // End of Clusters============================================================
  ...
  // Start of Space Points======================================================
        ...
        //   isMember("sci_fi_space_points")) {
          ...
          double space0_size = scuts["tracker0"].size();
...
// Start Efficiency Calculation=================================================
    ...
    space_eff1 = space_count1/(cut_recon_size*1.0);
    space_eff2 = space_count2/(cut_recon_size*1.0);
    cluster_eff1 = cluster_count1/(cut_recon_size*3.0);
    cluster_eff2 = cluster_count2/(cut_recon_size*3.0);
    pyfile << std::endl << "  Total cluster efficiency: Tracker 1: "
      << cluster_eff1 << "   Tracker 2: " << cluster_eff2 << std::endl;
    pyfile << "  Total space point efficiency: Tracker 1: " << space_eff1
      << "   Tracker 2: " << space_eff2 << std::endl << std::endl;
    Tspace_eff = ((Tspace_eff * (_nSpills-1)) + space_eff1) / (_nSpills);
    Tcluster_eff = ((Tcluster_eff * (_nSpills-1)) + cluster_eff1) / (_nSpills);
    _other.Fill();
    pyfile.close();
...
/*  c1->cd(1);
  ...
  c1->cd(2);
  _other.Draw("Tcluster_eff:nSpills");
  c1->cd(3);
  _other.Draw("space_eff1:nSpills");
  c1->cd(4);
  _other.Draw("Tspace_eff:nSpills");
...
void ReduceCppTrackerMCSingleStation::Save() {
...
bool ReduceCppTrackerMCSingleStation::death()  {
  Save();
  return true;
}
