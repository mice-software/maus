...
#        self.run_ended = False
	...
	print "print 1"
	    ...
	    self.text_match_eff.SetText(self.hrange_primary.GetXaxis().GetXmax(),
	    ...
	    if (chi2 > 5.1):
            ...
            for pl in range(len(spill['recon_events'][evn]['emr_event']\
				     ['emr_plane_hits'])):
                pl_id = spill['recon_events'][evn]['emr_event']\
			     ['emr_plane_hits'][pl]['plane']
                for bar in range(len(spill['recon_events'][evn]['emr_event']\
				     ['emr_plane_hits'][pl]['emr_bars'])):
		    bar_id = spill['recon_events'][evn]['emr_event']\
				  ['emr_plane_hits'][pl]['emr_bars']\
				  [bar]['bar']
		    for hit in range(len(spill['recon_events'][evn]\
				         ['emr_event']['emr_plane_hits'][pl]\
					 ['emr_bars'][bar]['emr_bar_hits'])):
                        tot = spill['recon_events'][evn]['emr_event']\
				   ['emr_plane_hits'][pl]['emr_bars']\
				   [bar]['emr_bar_hits'][hit]['tot']
		        if tot:
			    if (pl_id % 2 == 0):
			        self.hoccupancy_x.Fill(pl_id, bar_id)
			    else:
			        self.hoccupancy_y.Fill(pl_id, bar_id)
	print "print 2"
    ...
    def __init_histos(self): #pylint: disable=R0201, R0914
        ...
        self.hspread_vs_density.GetYaxis().SetTitle("#chi_{X}^{2}/N + #chi_{Y}^{2}/N")
	...
	# define text comment
        ...
        self.canvas_recon = ROOT.TCanvas("emr_recon", "emr_recon", 1200, 900)
        ...
        self.canvas_occ = ROOT.TCanvas("emr_occupancy", "emr_occupancy", 1000, 600)
    ...
    def get_histogram_images(self):       
        ...
        """
	print "get_histogram_images START"
        image_list = []
	...
	print "get_histogram_images END"
