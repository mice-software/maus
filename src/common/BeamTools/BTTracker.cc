                          ...
                          CLHEP::Hep3Vector origin, CLHEP::HepRotation rot)
{
  ...
  if( std::isnan(_m) || std::isinf(_m) ) throw(Squeal(Squeal::recoverable, "Mass undefined in stepping function", "BTTracker::integrate"));
...
void BTTracker::symplecticIntegrate(double end_tau, double* x_in, const BTField* field, double start_tau, double step_size, double charge, int order)
{
  ...
  if(end_tau-tau > step_size/100.)
    throw(Squeal(Squeal::recoverable, "Error during tracking", "BTTracker::symplecticIntegrate"));
                ...
                double step_size, BTSpaceChargeField* spaceCharge)
{
  ...
  switch (indep)
  {
    ...
    default:
    { throw(Squeal(Squeal::recoverable, "Only integration through time implemented in integrate many", "BTTracker::integrateMany") ); }
  }
