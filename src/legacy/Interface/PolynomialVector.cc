...
void PolynomialVector::SetCoefficients(int pointDim, MMatrix<double> coeff)
{
  ...
  for(size_t i=0; i< coefficient_matrix_.num_row(); i++) 
    for(int j=0; j<nPCoeff; j++)
      polynomial_vector_.push_back( PolynomialVector::PolynomialCoefficient(index_key_by_vector_[j], i,  coefficient_matrix_(i+1, j+1) ) );
}
...
void PolynomialVector::SetCoefficients(std::vector<PolynomialCoefficient> coeff)
{
  ...
  point_dimmension_++; //PointDim indexes from 0
   coefficient_matrix_ = MMatrix<double>(valueDim+1, NumberOfPolynomialCoefficients(point_dimmension_, maxPolyOrder+1), 0.);

  for(size_t i=0; i< coefficient_matrix_.num_col(); i++)
    index_key_by_power_.push_back(IndexByPower (i, point_dimmension_));
  for(size_t i=0; i< coefficient_matrix_.num_col(); i++) 
  {
    index_key_by_vector_.push_back(IndexByVector(i, point_dimmension_));
...
{
  for(size_t i=0; i<coeff.num_row() && i< coefficient_matrix_.num_row(); i++)
    for(size_t j=0; j<coeff.num_col() && j< coefficient_matrix_.num_col(); j++)
       coefficient_matrix_(i+1,j+1) = coeff(i+1,j+1);
}
...
{
    MVector<double> polyVector(index_key_by_vector_.size(), 1); 
    MakePolyVector(point, polyVector);
    MVector<double> answer =  coefficient_matrix_*polyVector;
...
double*  PolynomialVector::MakePolyVector_G4MICE(const double* point, double* polyVector) const
{
    for(unsigned int i=0; i<index_key_by_vector_.size(); i++)
    {
        polyVector[i] = 1.;
                                                              ...
                                                              int polynomialOrder, const std::vector<double>& weights)
{
  ...
  for(int i=0; i<nPoints; i++)
  {
    temp->MakePolyVector_G4MICE(&points[i][0], &tempFx[0]);
    for(int k=0; k<nCoeffs;  k++) 
      for(int j=0; j<nCoeffs;  j++) 
        F2(j+1, k+1) += tempFx[k]*tempFx[j]*wt[i];
    for(int j=0; j<nCoeffs; j++)
                                    ...
                                    const std::vector<double>& weights)
{
  ...
  std::vector< std::vector<double> > tempFx(nPoints, std::vector<double>(nCoeffsNew) );
  for(int i=0; i<nPoints; i++)
    newPolyVector1D.MakePolyVector_G4MICE(&points[i][0], &tempFx[i][0]);
