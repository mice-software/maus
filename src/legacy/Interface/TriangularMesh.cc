...
{///nd
	if(numberOfDimensions==2)
	{
		...
		throw(Exception(Exception::recoverable, "TriangularMesh only implemented for 2d", "TriangularMesh::TriangularMesh(...)"));
}
...
void TriangularMesh::ReadPointsFromQHull(std::string fileName)
{
	...
	if(!fin) throw(Exception(Exception::recoverable, "Error reading file", "QHullIO::ReadQHullInputFile"));
	fin.close();
}
...
void TriangularMesh::WriteQHullInput(std::string fileName)
{
	...
	if(!fout) throw(Exception(Exception::recoverable, "Error writing file", "TriangularMesh::WriteQHullInput"));
	fout.close();
}
...
TriangularMesh::Simplex& TriangularMesh::Simplex::FindLeaf(Point& test)
{
	...
	throw(Exception(Exception::recoverable, "Failed to find test point", "Simplex::FindLeaf"));
}
...
void TriangularMesh::Simplex::BarycentricCoordinates(Point& test, Point& apex, double barys[]) const
{
	...
	else throw(Exception(Exception::recoverable, "Trying to find BarycentricCoordinates with point not in Simplex", "Simplex::BarycentricCoordinates"));
