              ...
              : _symmetry(none)
{
	...
	if(fileType!="g4bl3dGrid") throw(Squeal(Squeal::recoverable, "Did not recognise file type \'"+fileType+"\'", "ThreeDFieldMap::ThreeDFieldMap"));
	fileName = ReplaceVariables(fileName);
	std::ifstream in(fileName.c_str());
	if(!in) throw(Squeal(Squeal::recoverable, "Could not open file \'"+fileName+"\'", "ThreeDFieldMap::ThreeDFieldMap"));
...
Interpolator3dGridTo3d* ThreeDFieldMap::LoadBeamlineMap(std::istream& fin, Interpolator3dGridTo3d::interpolationAlgorithm algo)
{
	...
	for(int i=0; i<6; i++)
	{
		...
		in >> name >> scaleStr;
		if(scaleStr.size() < 2) 
			throw( Squeal(Squeal::recoverable, "Scale \'"+scaleStr+"\' formatting not recognised", "ThreeDFieldMap::LoadBeamlineMap") );
  ...
  while( pos < fileName.size() )
  {
    if( fileName[pos] == '$' ) 	// we've found an environment variable 
    {
      ...
      if(getenv( variable.c_str() ) == NULL) 
          throw(Squeal(Squeal::recoverable, "Error - "+variable+" environment variable was not defined", "ThreeDFieldMap::ReplaceVariables"));
