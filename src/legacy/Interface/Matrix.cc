  ...
  const ::CLHEP::HepMatrix& hep_matrix ) : matrix_(NULL)
{
  ...
  for(size_t row=1; row<=rows; ++row)
  {
    for(size_t column=1; column<= columns; ++column)
    {
      (*this)(row,column) = Complex::complex(hep_matrix(row,column));
    }
  }
}
...
template MatrixBase<double, gsl_matrix>::Row<double, gsl_matrix>
MatrixBase<double, gsl_matrix>::operator[](const size_t row);
template
MatrixBase<complex, gsl_matrix_complex>::Row<complex, gsl_matrix_complex>
MatrixBase<complex, gsl_matrix_complex>::operator[](const size_t row);
...
template const MatrixBase<double, gsl_matrix>::Row<double, gsl_matrix>
MatrixBase<double, gsl_matrix>::operator[](const size_t row) const;
template
const MatrixBase<complex, gsl_matrix_complex>::Row<complex, gsl_matrix_complex>
MatrixBase<complex, gsl_matrix_complex>::operator[](const size_t row) const;
...
MatrixBase<complex, gsl_matrix_complex>::operator/=(const complex& rhs)
{
  gsl_matrix_complex_scale(matrix_, Complex::complex(1.)/rhs);
  return *this;
}
  ...
  : MatrixBase<complex, gsl_matrix_complex>()
{
  ...
  for(size_t row=1; row<=rows; ++row)
  {
    for(size_t column=1; column<=columns; ++column)
    {
      (*this)(row, column) = Complex::complex(real_matrix(row, column));
    }
  }
}
...

{
  ...
  {
    throw(Squeal(Squeal::recoverable,
                 "Attempted to build a complex matrix using real and imaginary matrices of different sizes",
                 "Matrix<complex>::Matrix()"));
  }
  build_matrix(rows, columns);
  for(size_t row=1; row<=rows; ++row)
  {
    for(size_t column=1; column<=columns; ++column)
    {
      (*this)(row, column) = Complex::complex(
                           ...
                           const gsl_matrix_complex * matrix_B)
{
  ...
  if (matrix_AB != NULL)
  {
    ...
    gsl_blas_zgemm(CblasNoTrans, CblasNoTrans,
                   Complex::complex(1.), matrix_A, matrix_B,
                   Complex::complex(0.), matrix_AB);
                           ...
                           gsl_vector_complex * vector_b)
{
  ...
  if (vector_Ab != NULL)
  {
    ...
    gsl_blas_zgemv(CblasNoTrans,
                   Complex::complex(1.), matrix_A, vector_b,
                   Complex::complex(0.), vector_Ab);
  ...
  const Matrix<double>& matrix)
{
  ...
  gsl_linalg_LU_decomp(copy.matrix_, p, &signum);
  double d = gsl_linalg_LU_det(copy.matrix_, signum);
  ...
  const Matrix<complex>& matrix)
{
  ...
  gsl_permutation * p = gsl_permutation_alloc (rows);
  Matrix<complex> copy(matrix);
  gsl_linalg_complex_LU_decomp(copy.matrix_, p, &signum);
  gsl_permutation_free(p);
  return gsl_linalg_complex_LU_det((gsl_matrix_complex*)copy.matrix_, signum);
}
  ...
  const Matrix<double>& matrix)
{
  ...
  for(size_t row=1; row<=rows; ++row)
  {
    for(size_t column=1; column<=columns; ++column)
    {
      ...
      if (test_value == std::numeric_limits<double>::infinity())
      {
        throw(Squeal(Squeal::recoverable,
          "Failed to invert matrix: result contained infinite elements - singular?",
          "MAUS::inverse()"));
      }
    }
  }
  ...
  const Matrix<complex>& matrix)
{
  ...
  for(size_t row=1; row<=rows; ++row)
  {
    for(size_t column=1; column<=columns; ++column)
    {
      ...
      if (   real(test_value) == std::numeric_limits<double>::infinity()
          || imag(test_value) == std::numeric_limits<double>::infinity())
      {
        throw(Squeal(Squeal::recoverable,
          "Failed to invert matrix: result contained infinite elements - singular?",
          "MAUS::inverse()"));
...
Vector<complex> eigenvalues(const Matrix<double>& matrix)
{
  ...
  Vector<complex> eigenvalue_vector((complex*) eigenvalues->data, rows);
  gsl_vector_complex_free(eigenvalues);
