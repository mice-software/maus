...
Spline1D::Spline1D (const Spline1D &s): grads_(0), secondDerivs_(0) {
  extent_ = s.extent_;
  int i;
...
Spline1D::Spline1D(): extent_(0), nodePoints_(0),
                            values_(0), grads_(0), secondDerivs_(0){}

void Spline1D::captureGrid ( int extent, const Data_t* nodes ) {

// Operations common to all the constructors of Spline1D:  Capture the
...
void Spline1D::captureValues ( int extent, const Data_t* values ) {
  values_ = new Data_t[extent_];
...
void Spline1D::fillValues ( int extent, double values_function(double x)) {
  values_ = new Data_t[extent_];
  double x;
  Data_t* vp = values_;
  for ( int i = 0; i < extent_; i++ ) {
...
void Spline1D::computeSecondDerivs () {
...
} // Spline1D - separate tables for values, grads
...
} // Spline1D - function returning value and grad supplied
...
double Spline1D::operator() ( double x ) const {
