...
void MagFieldMap::ReadMap(const std::string& mapFile, const std::string& fileType, std::string interpolation)
{
	...
	else throw(Exception(Exception::recoverable, "Field map type "+fileType+" not supported", "MagFieldMap::ReadMap"));
	if (!fMap.is_open()) throw(Exception(Exception::recoverable, "Error opening field map file"+myFileName, 
	                                                       "MagFieldMap::ReadMap"));
...
SplineInterpolator MagFieldMap::ReadG4BeamlineMap(std::ifstream &fMap)
{
  ...
  fMap >> dummy >>
	...
	if(!fMap) 
    throw(Exception(Exception::recoverable, "There was a problem while reading a fieldmap.", "MagFieldMap::ReadG4BeamlineMap" ));
...
//	Squeak::mout(Squeak::debug) << "icool field parameters nZ:" << numberOfZCoords << " dZ: " << dZ << " zMin: " << myZMin << " zMax: " << myZMax << " nR: "
...
SplineInterpolator MagFieldMap::ReadGregoireMap(std::ifstream &fMap)
{
	...
	if(!fMap)
    throw(Exception(Exception::recoverable, "There was a problem while loading a field map", "MagFieldMap::LoadGregoireMap"));
