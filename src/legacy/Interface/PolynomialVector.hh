...
public:
    ...
    MMatrix<double>                    GetCoefficientsAsMatrix() const {return  coefficient_matrix_;}
    /// Return the coefficients as a vector of PolynomialCoefficients, including 0 values.
    std::vector<PolynomialCoefficient> GetCoefficientsAsVector() const {return polynomial_vector_;}
    ...
    unsigned int PointDimension()         const {return point_dimmension_;}
    /// Length of the output value (y) vector.
    unsigned int ValueDimension()         const {return  coefficient_matrix_.num_row();}
    /// Index of highest power - 0 is const, 1 is linear, 2 is quadratic etc...
    unsigned int PolynomialOrder()        const {if(index_key_by_vector_.size()>0) return index_key_by_vector_.back().size()+1; else return 0;}
    ...
    /// PolyVector should be of size NumberOfPolynomialCoefficients().
    /// Could be static but faster as member function (use lookup table for _polyKey).
    double* MakePolyVector_G4MICE(const double* point, double* polyVector) const;
...
private:
    ...
    std::vector< std::vector<int> >    index_key_by_power_; //std::vector<int>[i_1] = j_1
    std::vector< std::vector<int> >    index_key_by_vector_; //std::vector<int>[j_1] = i_1
    MMatrix<double>                    coefficient_matrix_;
    std::vector<PolynomialCoefficient> polynomial_vector_;
    static bool                        print_headers_;
};
