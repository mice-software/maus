                                              ...
                                              _needsPhases(false), _amalgamatedFields(0)
{
	if(GetGridDefault()[0]<2 || GetGridDefault()[1]<2 || GetGridDefault()[2]<2) 
		throw(Exception(Exception::recoverable, "FieldGridX, FieldGridY, FieldGridZ datacards must be > 1", "BTFieldConstructor::BTFieldConstructor"));
...
void BTFieldConstructor::BuildFields(MiceModule * rootModule)
{
	...
	for(int fields = 0; fields < _numberOfFieldTypes; fields++)
	{
		...
		catch(Exception exception) {std::cerr << "Error while loading fields from module "+daughterModules[i]->fullName() << std::endl; exception.Print(); exit(1);}
		catch(...)          
  ...
  _electroMagneticField->Close();
	...
	if(_amalgamatedFields != ignoredAmalgamated)
	{
		...
		throw(Exception(Exception::recoverable, in.str(), "BTFieldConstructor()"));
	}
	}
	catch(Exception exception) {exception.Print(); exit(1);}
...
BTField * BTFieldConstructor::GetSolenoid(const MiceModule * theModule)
{
	...
	if(solenoidMode == "WriteDynamic")
		...
		else
		{
			...
			throw(Exception(Exception::recoverable, "Solenoid mode is WriteDynamic but no tolerance set in module "+theModule->name(), 
...
BTField * BTFieldConstructor::GetRFCavity(const MiceModule * theModule)
{
	...
	if(pillBoxMode == "Electrostatic")
	{
		...
		if(theModule->propertyStringThis("FieldType") == "RFFieldMap")
			throw(Exception(Exception::recoverable, "Can't have electrostatic RFFieldMap", "BTFieldConstructor::GetRFCavity"));
	}
	...
	if(newPillBox==NULL)
		throw(Exception(Exception::recoverable, "Unrecognised CavityMode", "BTFieldConstructor::GetRFCavity" ));
