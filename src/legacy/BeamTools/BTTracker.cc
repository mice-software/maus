                          ...
                          CLHEP::Hep3Vector origin, CLHEP::HepRotation rot)
{
  ...
  if( std::isnan(_m) || std::isinf(_m) ) {
    if (y[4]*y[4] - y[5]*y[5] - y[6]*y[6] - y[7]*y[7] > -1e-6) {
      ...
      throw(Exception(Exception::recoverable, "Mass undefined in stepping function", "BTTracker::integrate"));
    }
  }
  ...
  while(fabs(indie-target_indie) > 1e-6)
  {
    ...
    {
      throw(Exception(Exception::recoverable, "Failed during tracking", "BTTracker::integrate") );
      break;
    }
    if(nsteps > _maxNSteps)
    {
      ...
      throw(Exception(Exception::recoverable, ios.str()+" Exceeded maximum number of steps\n", "BTTracker::integrate") );
      break;
    }
  }
...
void BTTracker::symplecticIntegrate(double end_tau, double* x_in, const BTField* field, double start_tau, double step_size, double charge, int order)
{
  ...
  if(end_tau-tau > step_size/100.)
    throw(Exception(Exception::recoverable, "Error during tracking", "BTTracker::symplecticIntegrate"));
                ...
                double step_size)
{
  ...
  switch (indep)
  {
    ...
    default:
    { throw(Exception(Exception::recoverable, "Only integration through time implemented in integrate many", "BTTracker::integrateMany") ); }
  }
  ...
  while(fabs(indie-target_indie) > 1e-6)
  {
    ...
    if(status != GSL_SUCCESS)
    {
      throw(Exception(Exception::recoverable, "Failed during tracking", "BTTracker::integrateMany") );
      break;
    }
    if(nsteps > _maxNSteps)
    {
