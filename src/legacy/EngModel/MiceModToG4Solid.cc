...
G4VSolid* buildSolid ( MiceModule* mod )
{
    ...
    throw(Exception(Exception::recoverable, "Volume "+mod->volType()+" not recognised in module "+mod->fullName(), 
...
//One for each supported G4Solid
	...
	G4VSolid * buildTube     ( MiceModule* mod )
	{
		...
		if(mod->dimensions().x() > mod->dimensions().y()) 
			throw(Exception(Exception::recoverable, "Inner radius > outer radius of tube made from module "+mod->fullName(), "MiceModToG4Solid::buildTube") );
	...
	G4VSolid * buildEllipticalCone   ( MiceModule* mod )
	{
		...
		if( mod->propertyExistsThis( "ZCut", "double" )) {
			if( mod->propertyDouble( "ZCut" )>0.) 
				...
				throw(Exception(Exception::recoverable, "Elliptical cone ZCut should be > 0.", "MiceModule::buildEllipticalCone"));
		}
	...
	G4VSolid * buildSphere   ( MiceModule* mod )
	{
		...
		if(phi.x() < 0.*deg || phi.x() > 360.*deg || phi.y() < 0.*deg || phi.y() > 360.*deg)
			throw(Exception(Exception::recoverable, "Phi out of range in sphere module "+mod->fullName(), "MiceModToG4Solid::buildSphere") );
		...
		if(theta.x() < 0.*deg || theta.x() > 180.*deg || theta.y() < 0.*deg || theta.y() > 180.*deg)
			throw(Exception(Exception::recoverable, "Theta out of range in sphere module "+mod->fullName(), "MiceModToG4Solid::buildSphere") );

		if( mod->dimensions().x() >= mod->dimensions().y() )
			throw(Exception(Exception::recoverable, "Inner radius > outer radius of sphere made from module "+mod->fullName(), "MiceModToG4Solid::buildSphere") );
		checkDim(mod, 2);
	...
	G4VSolid * buildTorus    ( MiceModule* mod )
	{
		...
		if(pRmin < 0.)    throw(Exception(Exception::recoverable, "Negative torus inner radius not allowed in module "+mod->fullName(), "MiceModToG4Solid::buildTorus"));
		if(pRmax < pRmin) throw(Exception(Exception::recoverable, "Torus outer radius should be > torus inner radius in module "+mod->fullName(), "MiceModToG4Solid::buildTorus"));
		if(pRmax > rCurv) throw(Exception(Exception::recoverable, "Torus outer radius should be < torus radius of curvature in module "+mod->fullName(), "MiceModToG4Solid::buildTorus"));
		if(angleStart < 0. || angleStart > 360.*deg) 
			throw(Exception(Exception::recoverable, "Torus start angle out of range in module "+mod->fullName(), "MiceModToG4Solid::buildTorus"));
	...
	{ 	
		...
		try{
			...
			if( widthX1 < 0. || widthX2 < 0. || heightY1 < 0. || heightY2 < 0. || lengthZ < 0. )  throw( Exception( Exception::recoverable, "Negative parameter for dimension in  "+mod->fullName(), "MiceModToG4Solid::buildTrapezoid"));
			return new G4Trd( mod->name() + "Trapezoid", widthX1, widthX2, heightY1, heightY2, lengthZ );
		}
		catch(Exception exception){
		      std::string error = exception.GetMessage();
		      throw(Exception(Exception::recoverable, "Error building a trapezoid: " + error + "\'", "MiceModToG4Solid::buildTrapezoid"));
		}
	}
	...
	G4VSolid * buildBoolean  ( MiceModule* mod )
	{
		...
		while(mod->propertyExistsThis(boolProp, "string"))
		{
			...
			else throw(Exception(Exception::recoverable, "Did not recognise boolean type "+type, 
	...
	MiceModule* getModule      (MiceModule* mod, std::string newModName)
	{
		...
		if(!fin) 
		{
			...
			{
				fin.close();
				throw(Exception(Exception::recoverable, "Failed to open either file "+file+" or file "+newModName+" for boolean "+mod->fullName(),
	...
	void checkDim(MiceModule* mod, size_t length) {
		...
		for(size_t i=0; i<length && i<3; i++)
			if(mod->dimensions()[i] <= 0.) {
				throw(Exception(Exception::recoverable, "Dimension "+axes[i]+" in module "+mod->fullName()+" out of range", "MiceModToG4Solid::checkDim"));
			}
	}
