...
Polycone::Polycone(MiceModule *theModule) : _zCoordinates(NULL), _rInner(NULL), _rOuter(NULL)
{
	...
	if(!fin)
	{
		throw(Exception(Exception::nonRecoverable, "Could not find polycone file "+fileName, "Polycone::Polycone(MiceModule)"));
		return;
	}
  ...
  while( pos < fileName.size() )
  {
	...
	++pos;
      ...
      if(getenv( variable.c_str() ) == NULL) 
          throw(Exception(Exception::recoverable, "Error - "+variable+" environment variable was not defined", "Polycone::replaceVariables"));
...
void Polycone::GetNumberOfCoordinates(std::istream & fin)
{
	...
	//skip rest of line
	std::getline(fin, key);
	if(!fin) throw(Exception(Exception::nonRecoverable, "Error in polycone input", "Polcone.cc::GetNumberOfCoordinates()"));
}
...
void Polycone::GetUnits(std::istream & fin)
{
	...
	//skip rest of line
	std::getline(fin, key);
	if(!fin) throw(Exception(Exception::nonRecoverable, "Error in polycone input", "Polcone.cc::GetUnits()"));
}
...
void Polycone::GetShellCoordinates(std::istream & fin)
{
	...
	if(!fin) throw(Exception(Exception::nonRecoverable, "Error in polycone input", "Polcone.cc::GetShellCoordinates()"));
...
void Polycone::GetFillCoordinates(std::istream & fin)
{
	...
	if(!fin) throw(Exception(Exception::nonRecoverable, "Error in polycone input", "Polcone.cc::GetFillCoordinates()"));
}
