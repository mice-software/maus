...
{
  if(in.size() < 1 || out.size() != in.size()) throw(Exception(Exception::recoverable, "Badly conditioned input", "TransferMapCalculator::GetPolynomialTransferMap"));
                                   ...
                                   int maxNumberOfSteps, PhaseSpaceVector refIn, PhaseSpaceVector refOut)
{
  ...
  if(!pvec) throw(Exception(Exception::recoverable, "Failed to make any polynomial fit at all - try tweaking control parameters", "TransferMapCalculator::GetSweepingPolynomialTransferMap"));
