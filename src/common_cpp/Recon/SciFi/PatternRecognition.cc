...
void PatternRecognition::track_processing(const int trker_no, const int n_points,
  ...
  if (strks.size() > 1 && n_points == 3) check_for_scattering(strks);
}
...
void PatternRecognition::check_for_scattering(
    std::vector<SciFiStraightPRTrack*> &strks) const {
  std::cerr << "Checking for scattering..." << std::endl;
  std::vector<SciFiStraightPRTrack*> checked_trks = strks;

  // Find all the 3point tracks
  std::vector<SciFiStraightPRTrack*> strks_3pt;
  for (auto trk : strks) {
    if (trk->get_num_points() == 3) {
      strks_3pt.push_back(trk);
    } else {
      checked_trks.push_back(trk);
    }
  }
  // Do any of the tracks share a spacepoint?
  SciFiStraightPRTrack* merged_track = NULL;
  for (size_t i = 0; i < strks_3pt.size(); ++i) {
    if (merged_track) {
      merged_track = NULL;
      continue;
    }
    for (size_t j = i + 1; j < strks_3pt.size(); ++j) {
      if (i == j) continue;
      std::vector<SciFiSpacePoint*> spnts1 = strks_3pt[i]->get_spacepoints_pointers();
      std::vector<SciFiSpacePoint*> spnts2 = strks_3pt[j]->get_spacepoints_pointers();
      SciFiSpacePoint* shared_spoint = NULL;
      for (auto sp1 : spnts1) {
        if (shared_spoint) break;
        for (auto sp2 : spnts2) {
          if (sp1 == sp2) {
            shared_spoint = sp1;
            break;
      ...
      // If a shared spoint has been found, combine the distinct spoints from each track and
      // try to fit a track to them with the LSQ fitter
      if (shared_spoint) {
        std::vector<SciFiSpacePoint*> combined_spnts = spnts1;
        std::vector<int> hit_station_numbers;
        for (auto sp2 : spnts2) {
          for (auto sp1 : spnts1) {
            if (sp1 != sp2) {
              combined_spnts.push_back(sp2);
            }
          }
        }
        // Check there is one and only one spacepoint from each station
        std::sort(combined_spnts.begin(), combined_spnts.end(), compare_spoints_ascending_station);
        bool duplicate_stations = false;
        for (size_t k = 1; k < combined_spnts.size(); ++k) {
          if (combined_spnts[k-1]->get_station() == combined_spnts[k-1]->get_station()) {
            duplicate_stations = true;
            break;
          }
        }
        // If we have spacepoints in every station, try the fit
        if ((!duplicate_stations) && combined_spnts.size() == 5) {
          merged_track = fit_straight_track(5, combined_spnts);
        }
      }
      if (merged_track) {
        checked_trks.push_back(merged_track);
        // We are going to throw away the seperate tracks which have now been combined, hence
    ...
    if (!merged_track) {
      checked_trks.push_back(strks_3pt[i]);
    }
  }
  strks = checked_trks;
}
...
bool PatternRecognition::find_dsdz(int n_points, std::vector<SciFiSpacePoint*> &spnts,
                                   ...
                                   SimpleLine &line_sz, TMatrixD& cov_sz, int &handedness) const {
