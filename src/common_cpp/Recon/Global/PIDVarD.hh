 ...
 *  PID variable class, for KL ADC charge product, derived
 *  from PIDBase
 */
...
namespace global {
  ...
  class PIDVarD : public PIDBase1D {
  public:
    /// Constructor to create a PDF
    explicit PIDVarD(std::string hypothesis, std::string unique_identifier);
    /// Constructor to use a PDF to perform PID analysis
    PIDVarD(TFile* file, std::string hypothesis, int minD, int maxD);
    /// Destructor
    ~PIDVarD();
  ...
  private:
    ...
    static const int minBinD = 0;
    /// Maximum value of range of PDF
    static const int maxBinD = 8000;
    /// Minimum value of cut for PID
    static const int minD = 0;
    /// Maximum value of cut for PID
    static const int maxD = 8000;
  };
}
}
}
