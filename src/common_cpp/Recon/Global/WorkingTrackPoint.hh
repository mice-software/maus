...
#ifndef COMMON_CPP_RECONSTRUCTION_WORKING_TRACK_POINT_HH
#define COMMON_CPP_RECONSTRUCTION_WORKING_TRACK_POINT_HH
...
/* @class WorkingTrackPoint a phase space vector with redundant t/E
 * and z/Pz coordinates as well as an ID that links the working track
 * to the detector that measured it.
 *
 ...
 * the mass.
 */
class WorkingTrackPoint : public MAUS::PhaseSpaceVector {
 public:
   ...
   */
  WorkingTrackPoint(const WorkingTrackPoint & original_instance);

  /* @brief  Base class copy constructor.
   */
  explicit WorkingTrackPoint(const PhaseSpaceVector & original_instance,
                             const double z = 0.0,
                             const Particle::ID pid = Particle::kNone);

  /* @brief  Vector<double> copy constructor.
   */
  explicit WorkingTrackPoint(const Vector<double> & original_instance,
                             const double z = 0.0,
                             const Particle::ID pid = Particle::kNone);
  ...
  */
  explicit WorkingTrackPoint(double const * const array, const double z = 0.0);

  /* @brief Reconstructed working track point constructor. The track
   * point was not generated by a detector, so an uncertanty matrix
   * must be supplied. The detector_id field is set to Detector::None.
   */
  WorkingTrackPoint(const double time, const double energy,
  ...
  WorkingTrackPoint & operator=(const WorkingTrackPoint& rhs);

  const bool operator==(const WorkingTrackPoint& rhs) const;
...
MAUS::MAUSPrimaryGeneratorAction::PGParticle PrimaryGeneratorParticle(
    const WorkingTrackPoint & point);

std::ostream& operator<<(std::ostream& out, const WorkingTrackPoint& vector);
