...
bool BetaEvolver::check_step_4_names() {
  ...
  BTFieldGroup* mag_field = dynamic_cast<BTFieldGroup*>(_field->GetMagneticField());
  std::string names[] = {
      "EndCoil2_US",
      "CenterCoil_US",
      "EndCoil1_US",
  ...
  if (mag_field->GetFields().size() != 12)
      throw(Exceptions::Exception(Exceptions::recoverable, "Failed to find the correct number of fields", "BetaEvolver::rescale_step_4_lattice"));
  for (size_t i = 0; i < 12; ++i) {
      std::string a_name = mag_field->GetFields()[i]->GetName();
      if (a_name != names[i])
          throw(Exceptions::Exception(Exceptions::recoverable, "Field name mismatch; expected "+names[i]+" but found "+a_name, "BetaEvolver::rescale_step_4_lattice"));
  }
  return true;
}

void BetaEvolver::rescale_step_4_lattice(double e2_us, double cc_us, double e1_us, double m2_us, double m1_us, double fc_us,
...
std::vector<double> BetaEvolver::integrate(double target_z, double z_in, double beta_0, double alpha_0, double p, double step_size) {
  _field = Globals::GetMCFieldConstructor();
  _p = p;
  ...
  while(fabs(z-target_z) > 1e-6) {
    ...
    {
      throw(Exceptions::Exception(Exceptions::recoverable, "Failed during tracking", "BTTracker::integrate") );
      break;
    }
    if(nsteps > _maxNSteps)
    {
      std::stringstream ios;
      ios << "Killing beta with step size " << h << " at step " << nsteps << " of " << _maxNSteps << "\n" 
          << "b: " << b[0] << " b': " << b[1] << std::endl;
      throw(Exceptions::Exception(Exceptions::recoverable, ios.str()+" Exceeded maximum number of steps\n", "BetaEvolver::integrate") );
      break;
    }
  }
...
int BetaEvolver::z_beta_differential_equation(double z, const double b[2], double dbdz[2], void* params) {
  ...
  dbdz[2] = 1./b[0];
  // std::cerr << "BETA STEP " << z << " kappa " << kappa << " b " << b[0] <<" " << b[1] << " b' " << dbdz[0] << " " << dbdz[1] << std::endl;
