...
void FieldPhaser::SetUp() {
    ...
    MAUSGeant4Manager::GetInstance()->GetPhysicsList()->BeginOfReferenceParticleAction();
}
...
void FieldPhaser::SetPhases() {
    ...
    try {
        ...
        while (_phaserVirtualPlanes.GetPlanes().size() > 0 && n_attempts < BTPhaser::GetInstance()->NumberOfCavities()*5) {
            ++n_attempts;
    ...
    Squeak::mout(Squeak::info) << "\nMade " << n_attempts << " attempts to phase " << BTPhaser::GetInstance()->NumberOfCavities() << " cavities with " << _phaserVirtualPlanes.GetPlanes().size() << " remaining" << std::endl;
