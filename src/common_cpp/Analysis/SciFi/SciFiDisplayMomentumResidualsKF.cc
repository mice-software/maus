...
SciFiDisplayMomentumResidualsKF::~SciFiDisplayMomentumResidualsKF() {
...
void SciFiDisplayMomentumResidualsKF::Fill() {
  mOf1->cd();
  if (mTree) {
    ...
    for ( size_t i = 0; i < GetData()->mDataKF.size(); ++i ) {
      ...
      if (mTrackData.TrackerNumber == 0) {
        ...
        mResidualPzT1Log->Fill(mTrackData.PzRec - mTrackData.PzMc);
...
void SciFiDisplayMomentumResidualsKF::Save() {
  if (mOf1) {
    mOf1->cd();
...
bool SciFiDisplayMomentumResidualsKF::SetUpRoot() {
  // Setup the output TFile
  mOf1 = new TFile("SciFiDisplayMomentumResidualsKFOutput.root", "recreate");
  // Set up the TTree
  ...
  mTree->Branch("PzMc", &mTrackData.PzMc, "PzMc/D");
  mTree->Branch("PxRec", &mTrackData.PxRec, "PxRec/D");
  mTree->Branch("PyRec", &mTrackData.PyRec, "PyRec/D");
  mTree->Branch("PtRec", &mTrackData.PtRec, "PtRec/D");
  mTree->Branch("PzRec", &mTrackData.PzRec, "PzRec/D");
  // Set up the residual histograms
  int res_n_bins = 100;
  mResidualPtT1 = new TH1D("ResidualPtT1", "T1 p_{t} Residual", res_n_bins, -20, 20);
  ...
  mResidualPtT2 = new TH1D("ResidualPtT2", "T2 p_{t} Residual", res_n_bins, -20, 20);
