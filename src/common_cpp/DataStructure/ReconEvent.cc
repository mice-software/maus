...
ReconEvent::ReconEvent() :_tof_event(NULL), _tracker_event(NULL),
...
ReconEvent& ReconEvent::operator=(const ReconEvent& recon) {
    ...
    if (_tracker_event != NULL) {
        delete _tracker_event;
    }
    _tracker_event = new TrackerEvent(*recon._tracker_event);
    if (_ckov_event != NULL) {
        delete _ckov_event;
    }
...
TrackerEvent* ReconEvent::GetTrackerEvent() const {
    return _tracker_event;
}

void ReconEvent::SetTrackerEvent(TrackerEvent* event) {
    if (_tracker_event != NULL) {
        delete _tracker_event;
    }
    _tracker_event = event;
}
