...
void PatternRecognition::make_5tracks(const int track_type, const int trker_no,
  ...
  if ( track_type == 2 ) {
    std::vector<SciFiSpacePoint*> current_spnts;
    make_tilted_helix(n_points, 0, ignore_stations, current_spnts, spnts_by_station, htrks);
  }
...
void PatternRecognition::make_tilted_helix(const int n_points, const int stat_num,
                                    const std::vector<int> ignore_stations,
                                    std::vector<SciFiSpacePoint*> &current_spnts,
                                    SpacePoint2dPArray &spnts_by_station,
                                    std::vector<SciFiHelicalPRTrack*> &htrks) const {
  ...
  SciFiHelicalPRTrack* trk = form_tilted_track(n_points, current_spnts);
...
//  int charge = - handedness;
  ...
  MAUS::ReduceCppTiltedHelix_NS::SciFiHelicalPRTrack *track = new MAUS::ReduceCppTiltedHelix_NS::SciFiHelicalPRTrack(-1, n_points, charge, pos_0, phi_0, c_trial,
                                                       line_sz, -1.0, phi_i, spnts, covariance);
  track->set_fit_parameters(c_trial.get_fit_parameters());
  return track;
}

SciFiHelicalPRTrack* PatternRecognition::form_tilted_track(const int n_points,
                                                    std::vector<SciFiSpacePoint*> spnts ) const {
  // SciFiTools::print_spacepoint_xyz(spnts);
  ...
  SimpleCircle c_trial;
  TMatrixD cov_circle(3, 3); // The covariance matrix for the circle parameters alpha, beta, gamma
  bool good_radius = false; // = LeastSquaresFitter::tilted_circle_fit(_sd_1to4, _sd_5, _R_res_cut, spnts, c_trial,
  //                                                  cov_circle);
