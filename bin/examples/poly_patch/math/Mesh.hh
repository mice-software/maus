...
public:
    ...
    inline int                         ToInteger()             const {return _mesh->ToInteger(*this);}
    inline std::vector<int>            State()                 const {return _state;}
    inline int&       operator[](int i)       {return _state[i]; }
    inline const int& operator[](int i) const {return _state[i]; }
...
public:
    ...
    inline void    SetX(int nXCoords, double * x) {_x = std::vector<double>(x,x+nXCoords);}
    inline void    SetY(int nYCoords, double * y) {_y = std::vector<double>(y,y+nYCoords);}
    ...
    inline int            PositionDimension() const {return 2;}
    inline int            ToInteger(const Mesh::Iterator& lhs) const {return (lhs.State()[0]-1)*(_ySize)+lhs.State()[1]-1;}

    inline void           SetConstantSpacing(bool spacing) {_constantSpacing = spacing;}
    void           SetConstantSpacing();
    inline bool           GetConstantSpacing() const       {return _constantSpacing; }
...
public:
    ...
    inline void    SetX(int nXCoords, double * x) {_x = std::vector<double>(x,x+nXCoords);}
    inline void    SetY(int nYCoords, double * y) {_y = std::vector<double>(y,y+nYCoords);}
    inline void    SetZ(int nZCoords, double * z) {_z = std::vector<double>(z,z+nZCoords);}
    ...
    Mesh::Iterator End()   const;
    virtual void   Position(const Mesh::Iterator& it, double * position) const;
    inline int            PositionDimension() const {return 3;}
    inline int            ToInteger(const Mesh::Iterator& lhs) const  {return (lhs.State()[0]-1)*_zSize*_ySize+(lhs.State()[1]-1)*_zSize+(lhs.State()[2]-1);}
...
public:
    ...
    inline void           SetCoord        ( int dimension, int nCoords, double * x) {_coord[dimension] = std::vector<double>(x, x+nCoords);}

    inline Mesh::Iterator Begin() const  {return Mesh::Iterator(std::vector<int>(_coord.size(), 1), this);}
    inline Mesh::Iterator End()   const  {std::vector<int> end(_coord.size(), 1); end[0] = _coord[0].size()+1; return Mesh::Iterator(end, this);}

    inline void           Position(const Mesh::Iterator& it, double * position) const {for(unsigned int i=0; i<it.State().size(); i++) position[i] = _coord[i][it[i]-1];}
    inline int            PositionDimension() const {return _coord.size();}
