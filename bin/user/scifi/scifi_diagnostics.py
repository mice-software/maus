    ...
    for i in range(int(start_num)-1, tree.GetEntries()):
        ...
        if spill.GetDaqEventType() == "physics_event":
            ...
            for i in range(spill.GetReconEvents().size()):
                ...
                sp_graphs = draw_spoints(c_sp_xyz, tkus.spoints_x, tkus.spoints_y,
                                         tkus.spoints_z, tkds.spoints_x,
                                         tkds.spoints_y, tkds.spoints_z)
                ...
                mg = draw_sz(tkus.seeds_z, tkus.seeds_s, tkds.seeds_z, tkds.seeds_s, \
                             tkus.helix_sz_fits, tkds.helix_sz_fits, c_trk_sz)
                ...
                if len(tkus.straight_xz_fits) > 0:
                    tkus_txz = tkus.straight_xz_fits[0].GetParameter(1)
                if len(tkus.straight_yz_fits) > 0:
                    tkus_tyz = tkus.straight_yz_fits[0].GetParameter(1)
                if len(tkds.straight_xz_fits) > 0:
                    tkds_txz = tkds.straight_xz_fits[0].GetParameter(1)
                if len(tkds.straight_yz_fits) > 0:
                    tkds_tyz = tkds.straight_yz_fits[0].GetParameter(1)

                print 'Straight TkUS Pat Rec tracks: ' + str(len(tkus.straight_xz_fits))
                print 'Straight TkDS Pat Rec tracks: ' + str(len(tkds.straight_xz_fits))
                ...
                print ', tkus_tx: ' + str(tkus_txz) + ', tkus_ty: ' + str(tkus_tyz)
                print 'tkds spacepoints: ' + str(len(tkds.spoints_z)),
                print ', tkds seeds: ' + str(tkds.num_sseeds),
                print ', tkds_tx: ' + str(tkds_txz) + ', tkds_ty: ' + str(tkds_tyz)
...
def draw_global_spoints(can, all_data):
    ...
    tkus_tpoints_x = all_data[6]
    tkus_tpoints_y = all_data[7]
    tkus_tpoints_z = all_data[8]
    tkds_tpoints_x = all_data[9]
    tkds_tpoints_y = all_data[10]
    tkds_tpoints_z = all_data[11]

    tof0_spoints_x = all_data[12]
    tof0_spoints_y = all_data[13]
    tof0_spoints_z = all_data[14]

    tof1_spoints_x = all_data[15]
    tof1_spoints_y = all_data[16]
    tof1_spoints_z = all_data[17]

    tof2_spoints_x = all_data[18]
    tof2_spoints_y = all_data[19]
    tof2_spoints_z = all_data[20]
    ...
    if (len(tkus_tpoints_x) > 0) and (len(tkus_tpoints_y) > 0):
        ...
        tkus_tp_xy_graph.SetMarkerStyle(5)
        tkus_tp_xy_graph.SetMarkerColor(ROOT.kBlue)
    ...
    if (len(tkds_tpoints_x) > 0) and (len(tkds_tpoints_y) > 0):
        ...
        tkds_tp_xy_graph.SetMarkerStyle(5)
        tkds_tp_xy_graph.SetMarkerColor(ROOT.kBlue)
    ...
    if (len(tkus_tpoints_x) > 0) and (len(tkus_tpoints_z) > 0):
        ...
        tkus_tp_xz_graph.SetMarkerStyle(5)
        tkus_tp_xz_graph.SetMarkerColor(ROOT.kBlue)
    ...
    if (len(tkds_tpoints_x) > 0) and (len(tkds_tpoints_z) > 0):
        ...
        tkds_tp_xz_graph.SetMarkerStyle(5)
        tkds_tp_xz_graph.SetMarkerColor(ROOT.kBlue)
