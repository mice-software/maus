...
def find_virtual_planes( filename, associate_tolerance, associate_max_attempts ) :
# Initialise the dictionary
  virtual_plane_dict = []
...
def run_full( **keywords ) :
  pass
def run_errors( **keywords ) :
  pass
def run_spa( **keywords ) :
  pass
def run_corrections( **keywords ) :
  pass
def run_virtuals( **keywords ) :
  virtual_bunches = []

# Set up the MC Extractor
  mc_extractor = extraction.virtual_hits()
  mc_extractor.add_cut( 'pid', keywords["cut_pid"], operator.ne )
  mc_extractor.add_cut( 'r', keywords["cut_radius"], operator.ge )
...
## Initialise the file reader with ALL the files.
  file_reader = extraction.maus_reader( keywords["maus_root_files"] )
  ...
  while file_reader.next_event() and file_reader.get_total_num_events() != keywords["max_num_events"] :
    try :
...
# Load Scifi and MC events
      mc_event = file_reader.get_event( 'mc' )
      mc_extractor.load_tracks( mc_event )
#      mc_cut = mc_extractor.is_cut()

      mc_extractor.load_tracks( mc_event )
      virtuals = mc_extractor.get_current_trackpoints()
      for plane, hit in virtuals :
        if len( virtual_bunches ) <= plane :
          for i in range( len( virtual_bunches ), plane ) :
            virtual_bunches.append( xboa.Bunch.Bunch() )
...
# Print out plots relating to a specfic virtual plane on the order of the user
  if len( keywords["inspect_planes"] ) > 0 :
    print "Attempting to save", len( keywords["inspect_planes"] ), "virtual planes."
    outfile = ROOT.TFile.Open( os.path.join( output_directory, "measure_emittance_inspection_planes.root" ), "RECREATE" )
    for num in inspect_planes :
      if num >= len( virtual_bunches ) or len( virtual_bunches[num] ) == 0 or virtual_bunches[num].bunch_weight < 1.0 : 
        print "ERROR: Could not inspect Virtual Plane:", num
        continue
      print_plots( virtual_bunches[num], "inspect_plane_" + str( num ) + "_" )
    outfile.Close()
    
# Enforce a transmission cut
  end_bunch = virtual_bunches[ len( virtual_bunches ) - 1 ]
  for i in range( len( virtual_bunches ) - 1 ) :
    virtual_bunches[i].transmission_cut( end_bunch )

# Ignore bunches that aren't possible to plot. ROOT can be quite picky about it's food...
  print_bunches = [ virtual_bunches[ i ] for i in range( len( virtual_bunches ) ) if virtual_bunches[i].bunch_weight() > 1.0 and len( virtual_bunches[i] ) > 1 ]

# Produce a text file of the virtual plane information
  if keywords["save_data"] :
    with open( os.path.join( output_directory, "measure_emittance_virtual_plane_data.dat" ), 'w' ) as outfile :
      outfile.write( '# Position (x,y,z)   -   Momentum(x,y,z,tot)  -  Energy  -  Emittance  -  Beta  -  Alpha  -  Buch Weight\n' )
      for bunch in print_bunches :
        means = bunch.mean( [ 'x', 'y', 'z', 'px', 'py', 'pz', 'p', 'energy' ] )
...
# Produce a Root file with MC information in graphs
  if keywords["save_graphs"] :
    outfile = ROOT.TFile.Open( os.path.join( keywords["output_directory"], "measure_emittance_graphs.root" ), "RECREATE" )
    canvas, hist, graph = xboa.Bunch.Bunch.root_graph( print_bunches, 'mean', ['z'], 'bunch_weight', [''], 'm', '' )
    graph.Write()
    canvas.Close()
    canvas, hist, graph = xboa.Bunch.Bunch.root_graph( print_bunches, 'mean', ['z'], 'emittance', ['x','y'], 'm', 'mm' )
    ...
    canvas.Close()
# This doesn't yet work properly...
# Currently testing with a bug fix
    canvas, hist, graph = xboa.Bunch.Bunch.root_graph( print_bunches, 'mean', ['z'], 'standard_deviation', ['pz'], 'm', 'MeV' )
    graph.Write()
    canvas.Close()  
    canvas, hist, graph = xboa.Bunch.Bunch.root_graph( print_bunches, 'mean', ['z'], 'mean', ['r'], 'm', 'm' )
...
if __name__ == "__main__":
  ...
  parent_options = argparse.ArgumentParser( add_help=False )

# PARENT PARSER

  parent_options.add_argument( '-s', '--save_summary', action='store_true', help='Save a file containing the output from the analsys script.' )
  parent_options.add_argument( '-v', '--save_variables', action='store_true', help='Save a python script containing all the variables used or calculated during the running of the analysis script.' )
  parent_options.add_argument( '--output_directory', default='./', help='Change the location of the output directory in which to store the produced files.', metavar='DIRECTORY' )
  parent_options.add_argument( '--max_num_events',type=int, default=-1, help='Specify the maxmum number of events to analyse. Useful if you only have large datasets.' )
  parent_options.add_argument( '--no_virtuals', action='store_true', help="Forces the script to perform Monte Carlo recostruction using the SciFi Hits objects, rather than Virtual Hits." )
  parent_options.add_argument( 'maus_root_files', nargs='+', help='List of Root files, generated by MAUS to analyse' )

# CUTS
  cuts = parent_options.add_argument_group( title='Cuts', description='Control cuts that can be applied the data being loaded.' )
...
# OTHER OPTIONS
  options = parent_options.add_argument_group( title='Options', description='Fine tune the behaviour of the script.' )
  ...
  options.add_argument( '--recon_station', type=int, default=1, help='Choose the reconstruction station in the trackers. This is the location that the emittance will be reconstructed for cooling calculations.', metavar='STATION' )
...
####################################################################################################
  ...
  operations = parser.add_subparsers( help='Select the mode of operation for the script.', dest='command' )
  full_parser = operations.add_parser( 'full', help='Run full emittance calculation routines on a small sample and print results to stdout.', parents=[parent_options] )
  errors_parser = operations.add_parser( 'errors', help='Run the emittance histogramming routines to estimate errors.', parents=[parent_options] )
  spa_parser = operations.add_parser( 'spa', help='Run the Single Particle Amplitude reconstruction routines (Slow!)', parents=[parent_options] )
  corrections_parser = operations.add_parser( 'corrections', help='Run the routines to generate the correction matrices.', parents=[parent_options] )
  virtuals_parser = operations.add_parser( 'virtuals', help='Run the virtual planes-only analysis.', parents=[parent_options] )


####################################################################################################
# FULL RECONSTRUCTION PASRSER

  full_parser.add_argument( '-r', '--save_residuals', action='store_true', help='Save plots of position and momentum residuals for the reconstructed trackpoints.' )
  full_parser.add_argument( '-e', '--save_emittance', action='store_true', help='Save emittance plots for the reference planes.' )
  full_parser.add_argument( '--correction_upstream', help='Enter the location of the upstream correction matrix to use in emittance calculations.', metavar='FILENAME' )
  full_parser.add_argument( '--correction_downstream', help='Enter the location of the downstream correction matrix to use in emittance calculations.', metavar='FILENAME' )
  full_parser.add_argument( '--correction_directory', default=None, help='Specify a directory in which the up- and downstream correction matrices can be found. They are expected to be named "measure_emittance_correction_matrix_up.dat, etc.' )


####################################################################################################
# ERRORS PASRSER

  errors_parser.add_argument( '--emittance_bin_size', type=int, default=2000, help='Choose the number of muons in each ensemble for histogrammed emittance calculations.', metavar='SIZE' )
  errors_parser.add_argument( '--correction_upstream', help='Enter the location of the upstream correction matrix to use in emittance calculations.', metavar='FILENAME' )
  errors_parser.add_argument( '--correction_downstream', help='Enter the location of the downstream correction matrix to use in emittance calculations.', metavar='FILENAME' )
  errors_parser.add_argument( '--correction_directory', default=None, help='Specify a directory in which the up- and downstream correction matrices can be found. They are expected to be names "measure_emittance_correction_matrix_up.dat, etc.' )

  errors_parser.add_argument( '-r', '--save_residuals', action='store_true', help='Save plots of position and momentum residuals for the reconstructed trackpoints.' )


####################################################################################################
# SPA PASRSER

  spa_parser.add_argument( '--correction_upstream', help='Enter the location of the upstream correction matrix to use in emittance calculations.', metavar='FILENAME' )
  spa_parser.add_argument( '--correction_downstream', help='Enter the location of the downstream correction matrix to use in emittance calculations.', metavar='FILENAME' )
  spa_parser.add_argument( '--correction_directory', default=None, help='Specify a directory in which the up- and downstream correction matrices can be found. They are expected to be names "measure_emittance_correction_matrix_up.dat, etc.' )
...
# VIRTUALS PASRSER
  ...
  virtuals_parser.add_argument( '-d', '--save_data', action='store_true', help='Save a data file storing information from the virtual planes for later analysis.' )
  virtuals_parser.add_argument( '-g', '--save_graphs', action='store_true', help='Save graphs plotting the energy, momentum, emittance and beta function of the beam through the cooling channel.' )
  virtuals_parser.add_argument( '-p', '--inspect_planes', default=[], type=int, nargs='+', help='Enter a list of integers. Virtual planes with these ID numbers will have a verbose selection of plots produced.' )
...
# Copy the stdout to a text file if asked
    ...
    if namespace.command == "full" :
      run_full( **vars( namespace ) )
    if namespace.command == "errors" :
      run_errors( **vars( namespace ) )
    if namespace.command == "spa" :
      run_spa( **vars( namespace ) )
    if namespace.command == "corrections" :
      run_corrections( **vars( namespace ) )
    if namespace.command == "virtuals" :
      run_virtuals( **vars( namespace ) )
    else :
